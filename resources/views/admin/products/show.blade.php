...
@section('content')
    <div class="px-6 py-4 space-y-2 text-center">
        <h1 class="my-4 font-serif text-3xl text-center text-sky-600 dark:text-sky-500">Bienvenido a {{$product->name}}</h1>
    ...
    <div class="flex flex-col max-w-xl px-8 py-4 mx-auto bg-white rounded shadow h-96 dark:bg-slate-800">
        <h2 class="text-xl text-slate-600 dark:text-slate-300 hover:underline">{{$product->name}}</h2>
        <h2 class="text-xl text-slate-600 dark:text-slate-300 hover:underline">{{$product->id}}</h2>
        <h2>{{$product->image}}</h2>
    </div>
